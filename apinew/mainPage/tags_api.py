...
data_path = os.path.join(os.path.dirname(__file__), 'spitScoll.json')  # Update path to JSON file
data = []
try:
...
@tags_api.route('/TagSearch', methods=['GET'])
def tag_search():
...
@tags_api.route('/MainSearch', methods=['GET'])
def main_search():
    tag = request.args.get('tag')
